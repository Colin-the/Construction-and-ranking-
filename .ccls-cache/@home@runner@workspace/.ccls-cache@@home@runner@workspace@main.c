...
int RuskeyWilliamsRecursive(int *alpha, int *beta, int n){
  ...
  if (alpha) for (int i = 0; i < n; i++) if (alpha[i] != 0) lenAlpha++; 
  if (beta) for (int i = 0; i < n; i++) if (beta[i] != 0) lenBeta++;
  ...
  // Case α = ε
  if (lenAlpha == 0) return n * RuskeyWilliamsRecursive(NULL, beta, n-1);


  // General case: permutation is α, then n, then β.
  // Let k = lenA be the position of n within the current permutation.
  // Recurse on the rotated suffix of length n−1:
  // newAlpha = σ(β)  (length n−1), newBeta = alpha[0..lenA−1]
  // size is (n−1)
  int *newAlpha = sigma(beta, lenBeta);     // produces an array of size lenB = n−1
  int *newBeta  = alpha;                // length lenA = k
  int rec = RuskeyWilliamsRecursive(newAlpha, newBeta, n-1);
  return (n - lenAlpha) + n * rec;

  //return n - lenAlpha + n * RuskeyWilliamsRecursive( sigma(beta, lenBeta), alpha, n);
}
...
int main(void){
  ...
  int alpha[] = {};
  int beta[] = {2,1,0};
  int rank = RuskeyWilliamsRecursive(NULL, beta, 3);
  printf("rank: %d\n", rank);
